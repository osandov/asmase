...
const unsigned char arch_seccomp_code[] = {
	/* prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0); */
	0x48, 0xc7, 0xc7, 0x26, 0x00, 0x00, 0x00,	/* movq $PR_SET_NO_NEW_PRIVS, %rdi */
	0x48, 0xc7, 0xc6, 0x01, 0x00, 0x00, 0x00,	/* movq $1, %rsi */
	0x48, 0x31, 0xd2,				/* xorq %rdx, %rdx */
	0x4d, 0x31, 0xd2,				/* xorq %r10, %r10 */
	0x4d, 0x31, 0xc0,				/* xorq %r8, %r8 */
	0x48, 0xc7, 0xc0, 0x9d, 0x00, 0x00, 0x00,	/* movq $SYS_prctl, %rax */
	0x0f, 0x05,					/* syscall */
	/* struct sock_filter filter = BPF_STMT(BPF_RET | BPF_K, SECCOMP_RET_TRAP); */
	0x48, 0x83, 0xec, 0x18,				/* subq $24, %rsp */
	0xc7, 0x44, 0x24, 0x10, 0x06, 0x00, 0x00, 0x00,	/* movl $0x6, 16(%rsp) */
	0xc7, 0x44, 0x24, 0x14, 0x00, 0x00, 0x03, 0x00,	/* movl $0x00030000, 20(%rsp) */
	/* struct sock_fprog prog = { .len = 1, .filter = &filter }; */
	0x66, 0xc7, 0x04, 0x24, 0x01, 0x00,		/* movw $1, (%rsp) */
	0x48, 0x8d, 0x7c, 0x24, 0x10,			/* leaq 16(%rsp), %rdi */
	0x48, 0x89, 0x7c, 0x24, 0x08,			/* movq %rdi, 8(%rsp) */
	/* seccomp(SECCOMP_SET_MODE_FILTER, 0, &prog); */
	0x48, 0xc7, 0xc7, 0x01, 0x00, 0x00, 0x00,	/* movq $1, %rdi */
	0x48, 0x31, 0xf6,				/* xorq %rsi, %rsi */
	0x48, 0x89, 0xe2,				/* movq %rsp, %rdx */
	0x48, 0xc7, 0xc0, 0x3d, 0x01, 0x00, 0x00,	/* movq $NR_seccomp, %rax */
	0x0f, 0x05,					/* syscall */
	0x48, 0x83, 0xc4, 0x18,				/* addq $24, %rsp */
};
const size_t arch_seccomp_code_len = sizeof(arch_seccomp_code);
